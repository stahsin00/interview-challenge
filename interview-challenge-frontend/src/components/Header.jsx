        ...
        <div className='bg-black border-gray-500 border-b w-full h-16 fixed top-0 flex items-center shadow-md z-40'>
            ...
            <div className="w-full h-full flex items-center justify-center gap-7">
              ...
              <div className='h-full relative flex'>
                <button className="text-[#f8ad2d] font-bold flex items-center" onClick={() => setIsFilterOpen(!isFilterOpen)}>FILTER<IoMdArrowDropdown className={`ml-1 transform transition-transform duration-200 ${isFilterOpen ? 'rotate-180' : ''}`}/></button>
              </div>
            </div>
            <button className="absolute right-5 bg-[#f8ad2d] text-black py-1 px-2 rounded-md text-xl flex items-center font-bold hover:bg-[#d78f26] transition-colors duration-200 ease-in-out" onClick={() => {setIsFormOpen(true)}}><FaPlus className="pr-1 font-bold text-xl"/>ADD</button>
        </div>
        <div className={`bg-black fixed top-16 w-full z-40 flex items-center justify-end border-gray-500 transition-all duration-200 ease-in-out overflow-hidden ${isFilterOpen ? 'border-b opacity-100' : 'h-0 opacity-0'}`}>
