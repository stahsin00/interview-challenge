...
function MovieModal({ selectedMovie, setSelectedMovie }) {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/movies/${selectedMovie.id}`,
        {
          method: 'DELETE',
          headers: {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 backdrop-blur z-50 flex justify-center items-center">
      <div className="bg-black shadow-md rounded-lg border border-gray-500 w-5/6 h-5/6 flex flex-col md:flex-row relative overflow-hidden">
        <div className="w-full md:w-1/2 p-5 relative">
          ...
          <div className="text-gray-600 mb-4 text-sm font-bold px-2 w-full h-14 md:h-44">
        ...
        <img
          ...
          alt="movie poster"
          className="w-full md:w-1/2 h-auto md:rounded-tr-lg md:rounded-br-lg absolute bottom-0 md:relative md:bottom-auto"
        ></img>
        <button
